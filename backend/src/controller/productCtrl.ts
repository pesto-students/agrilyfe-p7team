...
const getaProduct = asyncHandler(async (req: any, res: any) => {
  const { id } = req.params;
  validateMongoDbId(id);
  try {
...
const getAllProduct = asyncHandler(async (req: any, res: any) => {
  try {
    // Filtering
...
const rating = asyncHandler(async (req: any, res: any) => {
  const { _id } = req.user;
