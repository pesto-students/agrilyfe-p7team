...
const getAllBlogs = asyncHandler(async (req: any, res: any) => {
  try {
...
const liketheBlog = asyncHandler(async (req: any, res: any) => {
...
const disliketheBlog = asyncHandler(async (req: any, res: any) => {
  const { blogId } = req.body;
  validateMongoDbId(blogId);
  // Find the blog which you want to be liked
  const blog = await Blog.findById(blogId);
